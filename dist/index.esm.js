...
reflectAttributes(){this.constructor._classProperties.forEach((a,b)=>{if(!a.reflect||"string"!=typeof b)return;const{prop2attr:c=identity}=a,d=this[b];null===d?this.removeAttribute(b):this.setAttribute(b,c.call(this,d));});}
...
,reflect:e=!1}=this.constructor._classProperties.get(a)||defaultPropertyDeclaration;d.call(this,b,c)&&(this.styleElement.innerHTML=this.updateStyle(),this.template.innerHTML=this.updateTemplate(),this.requestRender(!0,!0,e));}requestRender(a,b,c){this.preRenderHook(a,b,c),this._rAFScheduled||(this._rAFScheduled=!0,requestAnimationFrame(()=>{this.render(a,b,c),this._rAFScheduled=!1;}));}
preRenderHook(){}
render(a,b,c){b&&this._renderRoot.appendChild(this.styleElement),a&&this._renderRoot.appendChild(this.template.content),c&&this.reflectAttributes();}}_defineProperty(BaseElement,"_classProperties",new Map);
...
export { BaseElement, attr2bool, bool2attr, debounce, defaultPropertyDeclaration, defineElement, identity, isDifferent, noop, property };
